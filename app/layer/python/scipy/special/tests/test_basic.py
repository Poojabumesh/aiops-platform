...
class TestFactorialFunctions:
    ...
    def test_factorialx_array_dimension(self, dim, exact):
        ...
        assert_allclose(
            special.factorialk(n, 3, exact=True), np.array(exp[3], ndmin=dim)
        )
    ...
    def test_factorialx_array_like(self, level, exact):
        ...
        assert_func(
            special.factorialk(n, 3, exact=True), np.array(exp_nucleus[3], ndmin=level)
        )
    ...
    def test_factorial_array_corner_cases(self, content, dim, exact, dtype):
        ...
        if not content:
            ...
            exact
            and not np.issubdtype(n.dtype, np.integer)
            and n.size
            and np.allclose(n[~np.isnan(n)], n[~np.isnan(n)].astype(np.int64))
        ):
