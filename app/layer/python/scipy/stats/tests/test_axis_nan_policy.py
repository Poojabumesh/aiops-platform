...
def _axis_nan_policy_test(
        ...
        with np.errstate(divide="ignore", invalid="ignore"):
            ...
            # of the output.
                ...
                if any([str(e).startswith(message) for message in too_small_messages]):
                    ...
                    [str(e).startswith(message) for message in inaccuracy_messages]
                ):
    ...
    if nan_policy == "raise" and not data_generator == "all_finite":
        ...
        with suppress_warnings() as sup, np.errstate(divide="ignore", invalid="ignore"):
            ...
            sup.filter(
                UserWarning, "Sample size too small for normal " "approximation."
            )
            res = unpacker(
                hypotest(*data, axis=axis, nan_policy=nan_policy, *args, **kwds)
            )
...
def test_keepdims(
    hypotest, args, kwds, n_samples, unpacker, sample_shape, axis_cases, nan_policy
):
...
def test_mean_mixed_mask_nan_weights(weighted_fun_name):
    ...
    with np.testing.suppress_warnings() as sup:
        ...
        if weighted_fun_name not in {"pmean", "gmean"}:
            ...
            res5 = weighted_fun_ma(
                a_masked4, weights=b_masked4, axis=axis, _no_deco=True
            )
...
@pytest.mark.parametrize(
    "dtype",
    (list(np.typecodes["Float"] + np.typecodes["Integer"] + np.typecodes["Complex"])),
)
