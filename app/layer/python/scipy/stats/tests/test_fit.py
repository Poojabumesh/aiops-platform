...
def test_cont_fit(distname, arg, method):
    if distname in failing_fits[method]:
        ...
        if xfail:
            ...
            msg += (
                " [Set environment variable SCIPY_XFAIL=1 to run this"
                " test nevertheless.]"
            )
            pytest.xfail(msg)
...
class TestGoodnessOfFit:
    ...
    def test_against_ks(self):
        ...
        res = goodness_of_fit(
            stats.norm, x, known_params=known_params, statistic="ks", random_state=rng
        )
    ...
    def test_against_cvm(self):
        ...
        known_params = {"loc": np.mean(x), "scale": np.std(x, ddof=1)}
        res = goodness_of_fit(
            stats.norm, x, known_params=known_params, statistic="cvm", random_state=rng
        )
    ...
    def test_against_anderson_case_0(self):
        ...
        res = goodness_of_fit(
            stats.norm, x, known_params=known_params, statistic="ad", random_state=rng
        )
    ...
    def test_filliben_property(self):
        ...
        res = stats.goodness_of_fit(
            stats.norm, x, statistic="filliben", random_state=rng
        )
        known_params = {"loc": 0, "scale": 1}
    ...
    def test_against_ppcc(self, case):
        ...
        x = rng.normal(size=n)
        res = stats.goodness_of_fit(
            stats.rayleigh, x, statistic="filliben", random_state=rng
        )
