...
def least_squares(
    ...
    if loss not in IMPLEMENTED_LOSSES and not callable(loss):
        raise ValueError(
            "`loss` must be one of {} or a callable.".format(IMPLEMENTED_LOSSES.keys())
        )
    ...
    if f0.ndim != 1:
        raise ValueError(
            "`fun` must return at most 1-d array_like. " "f0.shape: {}".format(f0.shape)
        )
    ...
    if callable(jac):
        ...
        if method == "lm":
            ...
            if jac != "2-point":
                warn(
                    "jac='{}' works equivalently to '2-point' "
    ...
    if verbose >= 1:
        ...
        print(
            "Function evaluations {}, initial cost {:.4e}, final cost "
            "{:.4e}, first-order optimality {:.2e}.".format(
