...
def group_delay(system, w=512, whole=False, fs=2 * pi):
    ...
    if np.any(singular):
        gd[singular] = 0
        warnings.warn(
            "The group delay is singular at frequencies [{}], setting to 0".format(
                ", ".join(f"{ws:.3f}" for ws in w[singular])
            ),
            stacklevel=2,
        )
        ...
        warnings.warn(
            ...
            around which a singularity may be present".format(
                ", ".join(f"{ws:.3f}" for ws in w[near_singular])
            ),
            stacklevel=2,
        )
...
def iirdesign(wp, ws, gpass, gstop, analog=False, ftype="ellip", output="ba", fs=None):
    ...
    if not analog:
        if fs is None:
            ...
            raise ValueError(
                "Values for wp, ws must be less than fs/2"
