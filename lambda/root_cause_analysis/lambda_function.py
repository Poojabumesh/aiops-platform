...
def classify_root_cause(anomaly_metrics, log_errors):
    ...
    if 'Error Rate' in str(anomaly_metrics.get('anomalies', [])):
        exception_errors = [e for e in log_errors if e['type'] in ['exception', 'connection']]
...
def trigger_rollback(cluster, service, reason):
    ...
    try:
        rollback_function = os.environ.get('ROLLBACK_FUNCTION_NAME', 'aiops-platform-rollback')
...
def send_rca_report(root_causes, remediation_results, anomaly_data):
    ...
    for rc in root_causes:
        evidence_str = "\n".join([f"    - {e['message'][:100]}" for e in rc['evidence'][:2]])
...
""")
    ...
    remediation_str = "\n".join([f"  - {detail}" for detail in remediation_results.get('details', [])])
...
def lambda_handler(event, context):
    ...
    try:
        ...
        primary_cause = sorted(root_causes, key=lambda x: {'CRITICAL': 0, 'HIGH': 1, 'MEDIUM': 2, 'LOW': 3}[x['severity']])[0]
